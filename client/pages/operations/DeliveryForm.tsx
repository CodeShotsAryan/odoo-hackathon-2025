  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      for (const item of selectedProducts) {
        await apiService.operations.delivery({
          reference: formData.reference,
          product_id: item.productId,
          qty: item.qty,
          location_src_id: parseInt(formData.from),
        });
      }
      
      addToast('Delivery created successfully!', ToastType.SUCCESS);
      navigate('/operations/deliveries');
    } catch (error) {
      addToast('Failed to create delivery', ToastType.ERROR);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in pb-20">
      <div className="flex items-center gap-2 mb-6 text-sm text-gray-500">
        ...
        <span>/</span>
        <span className="font-medium text-gray-900 dark:text-white">New Delivery</span>
      </div>
