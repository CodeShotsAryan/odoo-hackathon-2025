...
const KanbanColumn = ({ title, items, colorClass }: { title: string, items: Receipt[], colorClass: string }) => (
  <div className="flex-1 min-w-[300px] flex flex-col gap-4">
    ...
    <div className="flex flex-col gap-3 h-full overflow-y-auto pb-10">
      {items.map((item) => (
        <div 
          key={item.id}
          className="bg-white dark:bg-dark-card p-5 rounded-xl border border-gray-200 dark:border-dark-border shadow-sm hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] dark:hover:shadow-[0_8px_30px_rgba(0,0,0,0.4)] hover:-translate-y-1 transition-all duration-300 cursor-pointer group animate-slide-up"
        >
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2 text-sm text-gray-900 dark:text-gray-100 font-medium mb-1">
               {item.from} <span className="text-gray-400">→</span> {item.to}
            </div>
...
const ReceiptsKanban: React.FC<ReceiptsKanbanProps> = ({ data }) => {
